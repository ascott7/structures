/**
 * \file 234tree-test.cpp
 * \author Andrew Scott
 *
 * \brief Tests an 234Tree for correctness using multiple types
 *
 * \details
 *   Configured to use the templated 234Tree found in 234tree.hpp as
...
TEST(234TreeIntTest, insertTests)
{
    234Tree<int> intTree;
    int test = 0;
...
/*
TEST(234TreeIntTest, basicEqualityTests)
{
    234Tree<int> intTree;
    234Tree<int> intTree2;
...
TEST(234TreeIntTest, copyConstructorTests)
{
    234Tree<int> intTree;
    int test = 120;
    intTree.insert(test);
    234Tree<int> intTree2{intTree};
...
TEST(234TreeIntTest, assignmentOperatorTests)
{
    234Tree<int> intTree;
    int test = 1234;
    intTree.insert(test);
...
TEST(234TreeIntTest, iteratorTests)
{
    234Tree<int> intTree;
    for (int i = 0; i < 100; ++i) {
        intTree.insert(i);
    ...
    int num = 0;
    for (234Tree<int>::iterator i = intTree.begin(); i != intTree.end(); ++i) {
        ASSERT_EQ(num, *i);
        ++num;
    }
}

TEST(234TreeIntTest, deleteElementTests) {
    234Tree<int> intTree;
    intTree.insert(5);
...
TEST(234TreeOtterTest, insertTests)
{
    234Tree<Otter> otterTree;
...
TEST(234TreeOtterTest, basicEqualityTests)
{
    234Tree<Otter> otterTree;
    234Tree<Otter> otterTree2;
    // check that empty trees are equal
...
TEST(234TreeOtterTest, copyConstructorTests)
{
    234Tree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
    234Tree<Otter> otterTree2{otterTree};
    // tests copy constructor copying one element tree
    ASSERT_EQ(otterTree, otterTree2);
    ...
    234Tree<Otter> otterTree3{otterTree2};
    // test copying a larger tree
    // also tests equality operator on larger trees
...
TEST(234TreeOtterTest, assignmentOperatorTests)
{
    234Tree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
...
TEST(234TreeOtterTest, iteratorTests)
{
    234Tree<Otter> otterTree;
    for (int i = 0; i < 100; ++i) {
    ...
    int num = 0;
    for (234Tree<Otter>::iterator i = otterTree.begin(); i != otterTree.end(); ++i) {
        *i;
        ++num;
    }
}

TEST(234TreeOtterTest, deleteElementTests) {
    234Tree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
    // just an assurance that the delete is actually changing the value of
