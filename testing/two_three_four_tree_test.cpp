/**
 * \file twothreefourtree-test.cpp
 * \author Rachel Lee and Andrew Scott
 *
 * \brief Tests a 2-3-4 Tree for correctness using multiple types
 *
 * \details
 *   Configured to use the templated 2-3-4 tree found in twothreefourtree.hpp as
 *   2-3-4 trees of different types
 *
 */
...
TEST(twothreefourTreeIntTest, insertTests)
{
...
/*
TEST(twothreefourTreeIntTest, basicEqualityTests)
{
    twothreefourTree<int> intTree;
    twothreefourTree<int> intTree2;
...
TEST(twothreefourTreeIntTest, copyConstructorTests)
{
    twothreefourTree<int> intTree;
    int test = 120;
    intTree.insert(test);
    twothreefourTree<int> intTree2{intTree};
    ...
    twothreefourTree<int> intTree3{intTree2};
...
TEST(twothreefourTreeIntTest, assignmentOperatorTests)
{
    twothreefourTree<int> intTree;
    int test = 1twothreefour;
    intTree.insert(test);
    twothreefourTree<int> intTree2;
    ASSERT_NE(intTree, intTree2);
    intTree2 = intTree;
    ...
    ASSERT_NE(intTree, intTree2);
    twothreefourTree<int> intTree3;
    ASSERT_NE(intTree2, intTree3);
    intTree3 = intTree2;
    ASSERT_EQ(intTree2, intTree3);
    intTree3.insert(1twothreefour5);
    ASSERT_NE(intTree2, intTree3);
}

TEST(twothreefourTreeIntTest, iteratorTests)
{
    twothreefourTree<int> intTree;
    for (int i = 0; i < 100; ++i) {
        intTree.insert(i);
    ...
    int num = 0;
    for (twothreefourTree<int>::iterator i = intTree.begin(); i != intTree.end(); ++i) {
        ASSERT_EQ(num, *i);
        ++num;
    }
}

TEST(twothreefourTreeIntTest, deleteElementTests) {
    twothreefourTree<int> intTree;
    intTree.insert(5);
...
TEST(twothreefourTreeOtterTest, insertTests)
{
    twothreefourTree<Otter> otterTree;
...
TEST(twothreefourTreeOtterTest, basicEqualityTests)
{
    twothreefourTree<Otter> otterTree;
    twothreefourTree<Otter> otterTree2;
    // check that empty trees are equal
...
TEST(twothreefourTreeOtterTest, copyConstructorTests)
{
    twothreefourTree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
    twothreefourTree<Otter> otterTree2{otterTree};
    // tests copy constructor copying one element tree
    ASSERT_EQ(otterTree, otterTree2);
    ...
    twothreefourTree<Otter> otterTree3{otterTree2};
    // test copying a larger tree
    // also tests equality operator on larger trees
...
TEST(twothreefourTreeOtterTest, assignmentOperatorTests)
{
    twothreefourTree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
    ...
    ASSERT_NE(otterTree, otterTree2);
    twothreefourTree<Otter> otterTree3;
...
TEST(twothreefourTreeOtterTest, iteratorTests)
{
    twothreefourTree<Otter> otterTree;
    for (int i = 0; i < 100; ++i) {
    ...
    int num = 0;
    for (twothreefourTree<Otter>::iterator i = otterTree.begin(); i != otterTree.end(); ++i) {
        *i;
        ++num;
    }
}

TEST(twothreefourTreeOtterTest, deleteElementTests) {
    twothreefourTree<Otter> otterTree;
    Otter phokey = Otter{"phokey"};
    otterTree.insert(phokey);
    // just an assurance that the delete is actually changing the value of
